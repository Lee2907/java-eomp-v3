...
function signIn(username, password) {
  ...
  fetch("https://lee-buka2907.herokuapp.com/auth", {
    method: "POST",
    body: JSON.stringify({
      username: `${username}`,
      password: `${password}`,
...
function signUp(name, surname, username, password, phone_no, user_email) {
  console.log(name);
  console.log(surname);
  ...
  fetch("https://lee-buka2907.herokuapp.com/registration/", {
    method: "POST",
    body: JSON.stringify({
      name: `${name}`,
      surname: `${surname}`,
    ...
    .then((data) => {
      ...
      products.forEach((product) => {
        ...
        document.querySelector(
          ".product-container"
        ).innerHTML += `<div class="product" id="${product.product_id}">
                          <div class="product_picture"><img class="image" src="${product.product_picture}" alt="${product[1]}" /></div>
                          <h3 class="product_name">${product.product_name}</h3>
                          <p class="price">${product.price}</p>
                          <p class="description">${product.description}</p>
                          <p class="quantity">${product.quantity}</p>
                          <p class="order_id">R${product.order_id}</p>
